class i{static containerId;container=null;static instances={};scrollbarWidth;constructor(s){this.container=document.querySelector(`#${s}`),this.scrollbarWidth=this.getScrollbarWidth()}static getInstance(s){return i.instances[s]||(i.instances[s]=new i(s)),i.instances[s]}getScrollbarWidth(){const s=document.createElement("div");s.style.visibility="hidden",s.style.overflow="scroll",document.body.appendChild(s);const a=document.createElement("div");s.appendChild(a);const t=s.offsetWidth-a.offsetWidth;return s.parentNode?.removeChild(s),t}lock(){this.container&&(this.container.style.overflow="hidden",this.container.style.paddingRight=`${this.scrollbarWidth}px`)}unlock(){this.container&&(this.container.style.overflow="",this.container.style.paddingRight="")}}class l{popUp;crissCross;overlay;body;ScrollLock;constructor(s,a,t){if(this.popUp=document.querySelector(`.${s}`),this.popUp===null)throw new Error(`Container with id #${this.popUp} not found.`);if(this.overlay=document.querySelector(`#${a}`),this.popUp===null)throw new Error(`Container with id #${this.popUp} not found.`);this.crissCross=this.popUp.querySelector(".pop-up__criss-cross"),this.crissCross?.addEventListener("click",()=>this.close()),this.body=document.querySelector("body"),this.ScrollLock=i.getInstance(t)}open(){this.ScrollLock.lock(),this.overlay.classList.add("overlay_display_block"),this.popUp.classList.add("pop-up_display_block"),window.setTimeout(()=>{this.overlay.classList.add("overlay_show"),this.popUp.classList.add("pop-up_open")},0),this.overlay.addEventListener("click",()=>this.close())}close(){this.popUp.classList.remove("pop-up_open"),this.overlay&&this.overlay.classList.remove("overlay_show");const s=()=>{this.overlay&&this.overlay.classList.remove("overlay_display_block"),this.popUp.classList.remove("pop-up_display_block"),this.ScrollLock.unlock(),this.popUp.removeEventListener("transitionend",s)};this.popUp.addEventListener("transitionend",s),this.overlay.removeEventListener("click",()=>this.close())}}class n extends l{constructor(s,a,t){super(s,a,t)}addInnerContent(s){const t=this.popUp.querySelector(".open-lobby-pop-up__title")?.querySelector(".open-lobby-pop-up__amount");t.innerHTML=s.rate;const e=this.popUp.querySelector(".open-lobby-pop-up__map-img");e.src=s.imgSrc,e.alt=s.map;const c=this.popUp.querySelector(".open-lobby-pop-up__flag-img");c.src=s.flagSrc;const d=this.popUp.querySelector(".open-lobby-pop-up__title-lobby");d.innerHTML=s.nameMatch,this.popUp.querySelector(".open-lobby-pop-up__btn").addEventListener("click",()=>{window.location.assign(`lobby?id=${s.id}`)})}}class p{container;options;popUp;constructor(s,a,t){this.container=document.querySelector(`#${s}`),this.options=a,this.popUp=t,this.render(),this.addEventHendler()}render(){const s=this.options.reduce((a,t)=>(a+=`<tr class="match-tr content__tr" id="${t.id}">
       ...
       </tr>`,a),"");this.container.innerHTML=s}addEventHendler(){this.container.addEventListener("click",s=>{const t=s.target.closest(".match-tr");if(t){const e=this.options.find(c=>c.id==t.id);e&&(this.popUp.addInnerContent(e),this.popUp.open())}})}}class h{container;options;popUp;constructor(s,a,t){this.container=document.querySelector(`#${s}`),this.options=a,this.popUp=t,this.render(),this.addEventHendler()}render(){const s=this.options.reduce((a,t)=>(a+=`<div class="match-card" id="${t.id}">
...
</div>`,a),"");this.container.innerHTML=s}addEventHendler(){this.container.addEventListener("click",s=>{const t=s.target.closest(".match-card");if(t){const e=this.options.find(c=>c.id==t.id);e&&(this.popUp.addInnerContent(e),this.popUp.open())}})}}export{l as B,p as M,n as O,h as a};
