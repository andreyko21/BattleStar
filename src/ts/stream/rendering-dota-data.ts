...
class RenderingDotaDataForStream {
  ...
  private async getData(): Promise<GetDotaStreamingDataQuery | undefined> {
    const ENDPOINT = 'https://battle-star-app.onrender.com/graphql';
    const params = this.getParamsQuery();
    try {
      const dota2BrodcastingsData = await request(
        ENDPOINT,
        GetDotaStreamingData,
        { id: params },
        {
  ...
  private async transformData(): Promise<{
    ...
    const abbrStreamingData = streamingData?.dota2Brodcasting?.data?.attributes;
    const streamingDataTransforming: {
      [key: string]: string | undefined;
    } = {
      title: abbrStreamingData?.title,
      country:
        abbrStreamingData?.ctreator?.data?.attributes?.Options?.selected_country
          ?.data?.attributes?.name,
      urlFlag:
        abbrStreamingData?.ctreator?.data?.attributes?.Options?.selected_country
          ?.data?.attributes?.flag.data?.attributes?.url,
      altTextFlag: abbrStreamingData?.ctreator?.data?.attributes?.Options
      ...
      typeLobby: abbrStreamingData?.participants.length.toString(),
      gameMode: abbrStreamingData?.gameModes?.data[0].attributes?.title,
...
import Sprite from './../../images/sprite.svg';
      ...
      <div class="stream-page__info stream-page__info_type_mode">
        ...
        <div class="stream-page__data-value"> ${streamingData.typeLobby}x${
          streamingData.typeLobby
        } </div>
      </div>
...
export { RenderingDotaDataForStream };
