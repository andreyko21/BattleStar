import{$ as n}from"./jquery-e7e79cb6.js";import{H as l,g as m,F as c,I as d,r as _,S as e}from"./header-fd6cb8f8.js";import{A as p}from"./sidebar-8f810a63.js";import{U as u}from"./default-avatar-dee36e0e.js";class g{header;teamData;sidebar;teamId;game;constructor(t,a,i){this.header=new l(t),this.sidebar=new p(a,i),this.teamId=m("id")||"",this.game=m("game")||""}async init(){try{if(!this.teamId)throw new Error("Team ID is not specified in the URL");const t=this.game==="dota2"?c:d;if(this.teamData=await _("https://battle-star-app.onrender.com/graphql",t,{id:this.teamId}),!this.teamData||!this.teamData[`${this.game}Team`]?.data&&!this.teamData.cs2Team.data){window.location.href="/teams.html";return}const a=new h(this.teamData[`${this.game}Team`]?.data||this.teamData.cs2Team.data);n(".team-page").append(a.getItemTemplate())}catch(t){console.error(t)}}}n(document).ready(()=>{new g("#wrapper","wrapper","КОМАНДЫ").init()});class h{id;attributes;constructor(t){this.id=t.id||"",this.attributes=t.attributes||{}}getTemplate(){const t=this.attributes.Team||{};let a="";return(this.attributes.awards?.data||[]).forEach(s=>{a+=`<img class="team-item__award-img" src="${s.attributes.logo.data[0].attributes.url}" alt="award">`}),`<a href="/team.html?id=${this.id}" class="teams-list__team-item team-item">
    ...
    </a>`}getItemTemplate(){const t=this.attributes.Team[0]||{};let a="";return(this.attributes.awards?.data||[]).forEach(s=>{a+=`<img class="team-item__award-img" src="${s.attributes.logo.data[0].attributes.url}" alt="award">`}),`
      <div class="team-page__container">
        ...
        <div class="team-page__stats">
              ...
              <svg class="stats-info-item__icon">
                <use class="cup" xlink:href="${e}#cup"></use>
              </svg>
    ...
    <div class="team-page__participants-list">
        ${this.attributes.players.data.map(s=>{const o=s.attributes.user?.data?.attributes.avatar?.data?.attributes.url||u;return`<a href="/user.html?id=${s.attributes.user?.data?.id||"0"}" class="team-page__participants-user-avatar">
    ...
    <p class="team-page__participants-rank">Ранг: 1320</p>
        ...
        <div class="team-page__nav">
              ...
              <svg class="stats-info-item__icon share">
                <use xlink:href="${e}#share"></use></svg
              >Поделиться
            </button>
