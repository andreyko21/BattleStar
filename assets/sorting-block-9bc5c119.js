import{S as l}from"./header-c8259d0f.js";class c{static containerId;container=null;static instances={};scrollbarWidth;constructor(t){this.container=document.querySelector(`#${t}`),this.scrollbarWidth=this.getScrollbarWidth()}static getInstance(t){return c.instances[t]||(c.instances[t]=new c(t)),c.instances[t]}getScrollbarWidth(){const t=document.createElement("div");t.style.visibility="hidden",t.style.overflow="scroll",document.body.appendChild(t);const s=document.createElement("div");t.appendChild(s);const e=t.offsetWidth-s.offsetWidth;return t.parentNode?.removeChild(t),e}lock(){this.container&&(this.container.style.overflow="hidden",this.container.style.paddingRight=`${this.scrollbarWidth}px`)}unlock(){this.container&&(this.container.style.overflow="",this.container.style.paddingRight="")}}class o{popUp;crissCross;overlay;body;ScrollLock;constructor(t,s,e){if(this.popUp=document.querySelector(`.${t}`),this.popUp===null)throw new Error(`Container with id #${this.popUp} not found.`);if(this.overlay=document.querySelector(`#${s}`),this.popUp===null)throw new Error(`Container with id #${this.popUp} not found.`);this.crissCross=this.popUp.querySelector(".pop-up__criss-cross"),this.crissCross?.addEventListener("click",()=>this.close()),this.body=document.querySelector("body"),this.ScrollLock=c.getInstance(e)}open(){this.ScrollLock.lock(),this.overlay.classList.add("overlay_display_block"),this.popUp.classList.add("pop-up_display_block"),window.setTimeout(()=>{this.overlay.classList.add("overlay_show"),this.popUp.classList.add("pop-up_open")},0),this.overlay.addEventListener("click",()=>this.close())}close(){this.popUp.classList.remove("pop-up_open"),this.overlay&&this.overlay.classList.remove("overlay_show");const t=()=>{this.overlay&&this.overlay.classList.remove("overlay_display_block"),this.popUp.classList.remove("pop-up_display_block"),this.ScrollLock.unlock(),this.popUp.removeEventListener("transitionend",t)};this.popUp.addEventListener("transitionend",t),this.overlay.removeEventListener("click",()=>this.close())}}class _ extends o{constructor(t,s,e){super(t,s,e)}addInnerContent(t,s){const a=this.popUp.querySelector(".open-lobby-pop-up__title")?.querySelector(".open-lobby-pop-up__amount");a.innerHTML=t.rate;const i=this.popUp.querySelector(".open-lobby-pop-up__map-img");i.src=t.imgSrc,i.alt=t.map;const n=this.popUp.querySelector(".open-lobby-pop-up__flag-img");n.src=t.flagSrc;const d=this.popUp.querySelector(".open-lobby-pop-up__title-lobby");d.innerHTML=t.nameMatch,this.popUp.querySelector(".open-lobby-pop-up__btn").addEventListener("click",()=>{window.location.assign(`lobby.html?${s}`)})}}class v{container;options;popUp;constructor(t,s,e){this.container=document.querySelector(`#${t}`),this.options=s,this.popUp=e,this.render(),this.addEventHendler()}render(){const t=this.options.reduce((s,e)=>(s+=`<tr class="match-tr content__tr" id="${e.id}">
         ...
         <td class="match-tr__participants">
           ...
           <div class="match-tr__rate-value"> ${e.mode}/${e.participants} </div>
         </td>
       ...
       </tr>`,s),"");this.container.innerHTML=t}addEventHendler(){this.container.addEventListener("click",t=>{const e=t.target.closest(".match-tr");if(e){const a=this.options.find(i=>i.id==e.id);a&&(this.popUp.addInnerContent(a,`game=cs2&id=${a.id}`),this.popUp.open())}})}}class m{container;options;popUp;constructor(t,s,e){this.container=document.querySelector(`#${t}`),this.options=s,this.popUp=e,this.render(),this.addEventHendler()}render(){const t=this.options.reduce((s,e)=>(s+=`<div class="match-card" id="${e.id}">
  ...
  <div class="match-card__content">
    ...
    <div class="match-card__other-data">
      <div class="match-card__data-block">
        ...
        <div class="match-card__participants">
          ...
          <div class="match-card__rate-value"> ${e.participants}/${e.mode} </div>
        </div>
      </div>
...
</div>`,s),"");this.container.innerHTML=t}addEventHendler(){this.container.addEventListener("click",t=>{const e=t.target.closest(".match-card");if(e){const a=this.options.find(i=>i.id==e.id);a&&(this.popUp.addInnerContent(a,`game=cs2&id=${a.id}`),this.popUp.open())}})}}class b{container;renderingTabs;constructor(t,s){if(this.container=document.querySelector(`#${t}`),this.container===null)throw new Error(`Container with id #${t} not found.`);this.renderingTabs=s,this.render()}createTabsTemplate(){return`
    ...
    ></table>`}render(){const t=`<div class="sorting-block__view-tabs view-tabs">
      ...
      </div>`,s=document.createElement("div");s.classList.add("sorting-block","tabs-block"),s.innerHTML=t,this.container?.append(s)}}export{o as B,v as M,_ as O,b as S,m as a};
